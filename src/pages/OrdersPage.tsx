...
export default function OrdersPage() {
    ...
    setForm(f => {
      ...
      if (exists >= 0) {
        const updated = f.items.map((item, idx) =>
          idx === exists ? { ...item, quantity: item.quantity + qty } : item
        );
        return { ...f, items: updated };
      }
      return { ...f, items: [...f.items, { productId: pid, quantity: qty }] };
    });
    setSelectedProductId("");
    setQty(1);
  };
  ...
  const handleDelete = async () => {
    ...
    try { await deleteOrder(deleteTarget.id); setDeleteTarget(null); loadOrders(); }
    catch { setError("Error al eliminar"); }
  };

  const filtered = filterStatus === "ALL" ? orders : orders.filter(o => o.status === filterStatus);
  const statusBadge = (s: OrderStatus) =>
    <span className={`badge bg-${STATUS_COLORS[s]}-subtle text-${STATUS_COLORS[s]} rounded-pill`}>{STATUS_LABELS[s]}</span>;
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body p-0">
          {loadingOrders
            ...
            : filtered.length === 0
              ? <div className="text-center py-5 text-muted"><i className="bi bi-cart3 fs-1 d-block mb-2"></i>No hay órdenes</div>
                  ...
                  <tbody>{filtered.map(o => (
                    <tr key={o.id}>
                      ...
                      <td className="text-end">
                        <button className="btn btn-sm btn-outline-primary me-2" onClick={() => openEdit(o)} title="Cambiar estado"><i className="bi bi-pencil"></i></button>
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg"><div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title fw-semibold">
              ...
              {editId !== null ? "Cambiar estado de orden" : "Nueva orden"}
            </h5>
          ...
          <div className="modal-body">
            ...
            {editId !== null && (
              <div className="alert alert-info small mb-3">
                ...
                Las órdenes existentes solo permiten actualizar su estado.
              </div>
            )}
            ...
            {editId === null && (
              <div className="border rounded-2 p-3 bg-light">
                <div className="fw-semibold small mb-3">
                  <i className="bi bi-list-ul me-2 text-primary"></i>Productos de la orden
                </div>

                {/* Tabla de items agregados */}
                {form.items.length > 0 && (
                  <table className="table table-sm mb-3 bg-white rounded-2">
                    <thead><tr><th>Producto</th><th style={{width:90}}>Cantidad</th><th style={{width:40}}></th></tr></thead>
                    <tbody>
                      {form.items.map((item, i) => (
                        <tr key={i}>
                          <td className="small align-middle">{getProductLabel(item.productId)}</td>
                          <td className="align-middle">{item.quantity}</td>
                          <td>
                            <button className="btn btn-sm btn-outline-danger py-0 px-1" onClick={() => removeItem(i)}>
                ...
                <div className="row g-2 align-items-end">
                  <div className="col">
                    <label className="form-label fw-semibold small mb-1">
                      ...
                      {loadingProducts && <span className="spinner-border spinner-border-sm ms-2"/>}
                    </label>
                    <select
                    ...
                    >
                      <option value="">
                        {loadingProducts ? "Cargando productos..." : products.length === 0 ? "No hay productos disponibles" : "-- Selecciona un producto --"}
                      </option>
                      {products.map(p => (
          ...
          <div className="modal-footer">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={saving || (editId === null && (!form.customerName || form.items.length === 0))}
            >
              {saving && <span className="spinner-border spinner-border-sm me-2"/>}
              {editId !== null ? "Actualizar estado" : "Crear orden"}
            </button>
          </div>
        </div></div>
      </div>}
      ...
      {deleteTarget && <div className="modal show d-block" style={{background:"rgba(0,0,0,0.4)"}}>
        <div className="modal-dialog modal-dialog-centered modal-sm"><div className="modal-content">
          <div className="modal-header border-0">
            <h5 className="modal-title fw-semibold text-danger"><i className="bi bi-exclamation-triangle me-2"></i>Eliminar</h5>
            <button className="btn-close" onClick={() => setDeleteTarget(null)}/>
          </div>
          <div className="modal-body pt-0">
            ¿Eliminar orden <strong>#{deleteTarget.id}</strong> de <strong>{deleteTarget.customerName}</strong>?
          </div>
