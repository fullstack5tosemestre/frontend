...
import type { Product, Warehouse, ProductInput } from "../types";
import {
    getProducts,
    createProduct,
    updateProduct,
...
export default function ProductsPage() {
    ...
    const openCreate = () => {
        ...
        setForm({
            ...empty,
            inWarehouse: defaultWarehouseId
    ...
    const openEdit = (p: Product) => {
        setForm({
            ...
            sku: p.sku,
            stock: p.stock,
            inWarehouse: p.inWarehouse ? { id: p.inWarehouse.id } : undefined,
        });
        setEditId(p.id);
        setShowModal(true);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            const payload: ProductInput = {
                ...form,
                inWarehouse: form.inWarehouse?.id
                    ? { id: form.inWarehouse.id }
                    : undefined,
            };
            if (editId !== null) {
                                ...
                                <div className="row g-3">
                                    ...
                                    <div className="col-12">
                                        ...
                                        <select
                                            ...
                                            value={form.inWarehouse?.id ?? ""}
                                            onChange={(e) =>
                                                setForm({
                                                    ...form,
                                                    inWarehouse: e.target.value
                                                        ? {
                                                              id: Number(
                                                                  e.target
                                                                      .value,
                                            ...
                                            {warehouses.map((w) => (
                                                <option key={w.id} value={w.id}>
                                                    {w.name ??
                                                        `Bodega #${w.id}`}
                                                    {w.inBranch
                                                        ? ` — ${w.inBranch.name ?? "Sucursal #" + w.inBranch.id}`
                                                        : ""}
                                                </option>
                                            ))}
                                        </select>
                                        <div className="form-text">
                                            {warehouses.length === 0
                                                ? "Debes crear una bodega antes de registrar productos."
                                                : "Selecciona la bodega donde se almacena este producto"}
                                        </div>
                                    </div>
                                </div>
                            </div>
